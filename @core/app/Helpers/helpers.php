...
function render_image_markup_by_attachment_id($id, $class = null, $size = 'full')
{
    ...
    if (empty($id)){
        ...
        $on_image_output_return = '<img src="'.$noImageMarkupRender.'" class="" alt=""/>';
    ...
    $noImageMarkupRender =  asset('assets/uploads/no-image.png');
     ...
     if (!empty($image_details)) {
             if(empty($image_details['img_url'])){
                $output = '<img src="'.$noImageMarkupRender.'" ' . $class_list . ' alt="' . $image_details['img_alt'] . '"/>';
                 return $output;
             }
...
function sellerServiceLocation($service){
    ...
    if (!empty(get_static_option('google_map_settings'))){
         $seller_service_location = optional(optional($service)->seller)->seller_address ?? '';
        return $seller_service_location;
    }else{
        // seller location get
