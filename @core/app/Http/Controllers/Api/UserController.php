...
class UserController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $user = User::select('id', 'email','phone','user_type', 'password','country_id','state','username')->where([$login_type => $request->email,'user_type' => $user_type])->first();
        ...
        */
        ...
        $request->validate([
            'phone' => 'required|string|max:15|regex:/^\+?(\d{1,3})?(\d{7,15})$/',
        ]);


        // Search for user based on phone number
        $user = User::where('phone', $request->phone)->first();

        // If the phone number is present, we send OTP
        if ($user) {
            ...
            return response()->success([
                'message' => __('تم ارسال كود التحقق بنجاح الى  رقم جوالك'),
                'otp'     => $otp,
            ]);
        } else {
            // If the phone number does not exist, we create a new account
            // Generate data for the new user randomly
            $name = 'User_' . Str::random(8);
            $email = Str::random(10) . '@example.com';
            $username = 'user_' . Str::random(6);

            // Verify the uniqueness of the email and username
            ...
            return response()->success([
                'message' => __('تم انشاء حسابك وتم ارسال كود التحقق الى رقم جوالك'),
                'otp'     => $otp,
            ]);
        }
    }
    ...
    public function verifyOTP(Request $request)
    {
        ...
        // التحقق من الكود المخزن في الـ Cache
        $cachedOTP = Cache::get('otp_' . $request->phone);

        if (!$cachedOTP) {
            return response()->error([
                'message' => __('OTP has expired or is invalid'),
            ]);
        }

        if ($cachedOTP != $request->otp) {
            return response()->error([
                'message' => __('كود التحقق غير صحيح'),
            ]);
        }


        $user = User::select('id', 'email','phone','user_type', 'password','country_id','state','username')->where('phone', $request->phone)->first();
    ...
    private function sendSMSOTP($phone, $otp)
    {
        ...
        $username = 'eMK3ZnPJmqT44xyUlsbJCgVvs0pGIG2icoRCH47R';
        $password = 'KenCmqJnEBULdI7Ve3sSYm9HabMjHr4TgfyXXvIMMuKjUtzYD5Bub09HvbU3xiQziWQOYISsUVSIK25zn8zuWCXFgQCSWNHjFZUB';
