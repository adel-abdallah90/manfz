...
//            event(new SupportMessage($ticket_info));
    ...
    public function extraServiceAccept(Request $request)
    {
        ...
        elseif ($selected_payment_gateway === 'zitopay'){
            ...
            try {
                return PaymentGatewayRequestHelper::kineticpay()->charge_customer($this->buildPaymentArg($extra_service_details,route('buyer.order.extra.service.payment.kineticpay.ipn')));
            }catch (\Exception $e){
                toastr_error($e->getMessage());
                return back();
            }
