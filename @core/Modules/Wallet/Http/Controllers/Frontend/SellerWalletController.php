...
class SellerWalletController extends Controller
{
    ...
    public function wallet_history(Request $request)
    {
        ...
        if(!empty($request->payment_id || $request->wallet_date || $request->status)){
            $wallet_history_query = WalletHistory::latest()->where('buyer_id',Auth::guard('web')->user()->id);

            if (!empty($request->payment_id)){
                $wallet_history_query->where('id', $request->payment_id);
            }
            if (!empty($request->wallet_date)){
                $start_date = \Str::of($request->wallet_date)->before('to');
                $end_date = \Str::of($request->wallet_date)->after('to');
                $wallet_history_query->whereBetween('created_at', [$start_date,$end_date]);
            }
            if (!empty($request->status)){
                $wallet_history_query->where('payment_status', $request->status);
            }
            $wallet_histories = $wallet_history_query->paginate(10);
            ...
            elseif($request->selected_payment_gateway === 'zitopay'){
                ...
                try{
                    ...
                    $redirect_url = $kineticpay->charge_customer([
                        ...
                        'title' => $title,
                        'description' => $description,
                        'ipn_url' => route('seller.kineticpay.ipn.wallet'),
                        'order_id' => $last_deposit_id,
                        'track' => \Str::random(36),
                        'cancel_url' => route(self::CANCEL_ROUTE,$last_deposit_id),
                        'success_url' => route('buyer.orders'),
