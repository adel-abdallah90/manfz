...
class BuySubscriptionController extends Controller
{
    ...
    public function buy_subscription(Request $request)
    {
        ...
        if(isset($request->subscription_id)){
            ...
            if ($request->price == 0){
                //todo check it price is zero, check it already has subscription for this month in zero price
                $check_already_has_free_subscription_in_this_month =  SubscriptionHistory::where([
                    'seller_id' => auth('web')->id(),
                    'subscription_id' => $request->subscription_id,
                    'payment_status' => 'complete',
                ])->limit(1)->latest()->first();//->whereMonth('expire_date',Carbon::today())->exists();

                $expire_date = Carbon::parse($check_already_has_free_subscription_in_this_month->expire_date)->subDays(2);
                if (Carbon::today()->lessThan($expire_date)){
                    toastr_error(__('you have already avail free plan in this month, you cannot avail free plan more than once in a month.'));
            ...
            if( $price == 0){
                $this->update_database($last_subscription_id, __('zero price'),$last_subscription_history);
                toastr_success(__('Subscription Complete..'));
                return back();
            }
            ...
            if($request->selected_payment_gateway === 'manual_payment') {
                ...
                elseif($request->selected_payment_gateway === 'zitopay'){
                    ...
                    $redirect_url = $kineticpay->charge_customer([
                        ...
                        'title' => $request->type,
                        'description' => 'Subscription',
                        'ipn_url' => route('seller.kineticpay.ipn.subs'),
                        'order_id' => $last_subscription_id,
                        'track' => \Str::random(36),
                        'cancel_url' => route(self::CANCEL_ROUTE,$last_subscription_id),
                        'success_url' => route(self::SUCCESS_ROUTE,$new_order_id),
